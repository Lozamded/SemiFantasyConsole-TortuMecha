...
from tortoisengine.bake import (
    bake_background,
...
from tortoisengine.cart_manifest import CartManifest, tileset_manifest_key
from tortoisengine.constants import SCREEN_HEIGHT, SCREEN_WIDTH
from tortoisengine.gui_layer import (
    FILL_BOTTOM_TO_TOP,
    FILL_RIGHT_TO_LEFT,
    FILL_TOP_TO_BOTTOM,
    REPEAT_VERTICAL,
...
from tortoisengine import localization
from tortoisengine.instance_scripts import InstanceScript, load_instance_script
from tortoisengine.object import TortoiseObject, load_object
from tortoisengine.palette import load_palette, palette_path
from tortoisengine.pip_bar import PipBar, load_pip_bar
from tortoisengine.progress_bar import ProgressBar, load_progress_bar
from tortoisengine.scene import EMPTY_TILE, Scene, SceneObject, tile_size_for_tile_layer
from tortoisengine.sprite import Sprite, load_sprite
from tortoisengine.sprite_font import (
    TortoiseSpriteFont,
    load_sprite_font,
...
from tortoisengine.text_font import (
    TortoiseFont,
    load_tortu_font,
    measure_text_width,
    render_text_block,
...
from tortoisengine.text_wrap import autofit_scale, wrap_text
...
class SceneRenderer:
    """Caches baked assets while rendering scenes for TortoisePlayer / TortoiseStudio preview."""
    ...
    def __init__(self, project_root: Path, *, cart_manifest: CartManifest | None = None) -> None:
        ...
        self._objects: dict[str, TortoiseObject] = {}
        self._gui_layers: dict[str, GuiLayer] = {}
        self._progress_bars: dict[str, ProgressBar] = {}
        self._pip_bars: dict[str, PipBar] = {}
        self._text_fonts: dict[str, TortoiseFont] = {}
        self._sprite_fonts: dict[str, TortoiseSpriteFont] = {}
    ...
    def _tortu_object(self, prefab_path: str) -> TortoiseObject | None:
        if not prefab_path:
            return None
        if self._cart_mode():
            manifest = self._cart_manifest
            ...
            if raw is None:
                return None
            from tortoisengine.object import ObjectAnimation, ObjectCollider, ObjectOrigin
    ...
    def _text_font(self, rel_path: str) -> TortoiseFont | None:
        if not rel_path:
            return None
    ...
    def _sprite_font(self, rel_path: str) -> TortoiseSpriteFont | None:
        if not rel_path:
            return None
    ...
    def _render_gui_label(
        ...
        # tortoisengine.text_wrap.autofit_scale), which re-wraps at each
