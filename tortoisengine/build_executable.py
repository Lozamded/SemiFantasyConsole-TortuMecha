...
from tortoiseplayer.__main__ import main
...
_PYINSTALLER_FLAGS = [
    "--onefile",
    "--console",
    "--collect-all", "tortoisengine",
    "--collect-all", "tortoiseplayer",
...
def _build_podman(
    ...
    with tempfile.TemporaryDirectory(prefix="tortu_podman_") as tmp:
        ...
        src = tmp_path / "src"
        src.mkdir()
        shutil.copytree(_PKG_ROOT / "tortoisengine", src / "tortoisengine")
        shutil.copytree(_PKG_ROOT / "tortoiseplayer", src / "tortoiseplayer")
        # Write a minimal pyproject.toml — no PyQt6/tortoisestudio inside the container
        (src / "pyproject.toml").write_text(
            ...
            'where = ["."]\n'
            'include = ["tortoisengine*", "tortoiseplayer*"]\n',
            encoding="utf-8",
        )
