"""Horizontal progress-bar prefab strip — pick a .tortuprogressbar to place as a tiled rect."""
...
class ProgressBarStripCanvas(QWidget):
    """Clickable strip of progress-bar prefab thumbnails (tiled texture preview)."""

    SELECTION_COLOR = (255, 220, 80)
    GRID_COLOR = (36, 36, 50)
    EMPTY_BG = (48, 48, 60)

    progress_bar_clicked = pyqtSignal(int)
    ...
    def _build_thumbnail(self, prefab_path: str) -> QImage | None:
        ...
        try:
            ...
            if not bar.texture:
                return None
            sprite_file = (self.project_root / bar.texture).resolve()
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        if not self.prefab_paths:
            ...
            painter.drawText(self.rect(), Qt.AlignmentFlag.AlignCenter, "No progress bars in project")
            painter.end()
            return
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:  # noqa: N802
        ...
        if index is not None:
            ...
            self.update()
            self.progress_bar_clicked.emit(index)
