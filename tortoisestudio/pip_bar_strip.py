"""Horizontal pip-bar prefab strip — pick a .tortupipbar to place as a repeat sprite."""
...
class PipBarStripCanvas(QWidget):
    """Clickable strip of pip-bar prefab thumbnails (icon preview)."""

    SELECTION_COLOR = (255, 220, 80)
    GRID_COLOR = (36, 36, 50)
    EMPTY_BG = (48, 48, 60)

    pip_bar_clicked = pyqtSignal(int)
    ...
    def _build_thumbnail(self, prefab_path: str) -> QImage | None:
        ...
        if not path.is_file():
            return None
        try:
            pip_bar = load_pip_bar(path)
            if not pip_bar.full_sprite:
                return None
            sprite_file = (self.project_root / pip_bar.full_sprite).resolve()
    ...
    def _sprite_thumbnail(self, sprite, palette: list[tuple[int, int, int]]) -> QImage:
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        if not self.prefab_paths:
            ...
            painter.drawText(self.rect(), Qt.AlignmentFlag.AlignCenter, "No pip bars in project")
            painter.end()
            return
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:  # noqa: N802
        ...
        if index is not None:
            ...
            self.update()
            self.pip_bar_clicked.emit(index)
