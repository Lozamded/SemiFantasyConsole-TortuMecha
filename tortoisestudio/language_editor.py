"""Language editor — edit the languages/*.csv translation tables in a grid.
...
class LanguageEditorWidget(QWidget):
    """File list of languages/*.csv on the left, a key/language grid on the right."""
    ...
    def __init__(self, project_root: Path, parent: QWidget | None = None) -> None:
        ...
        file_col.addWidget(QLabel("languages/*.csv"))
    ...
    def refresh(self) -> None:
        languages_dir = self.project_root / LANGUAGES_DIR
        files = sorted(languages_dir.glob("*.csv")) if languages_dir.is_dir() else []
        ...
        self.status_label.setText(
            f"{count} file{'s' if count != 1 else ''} in {LANGUAGES_DIR.as_posix()}"
        )
    ...
    def _new_file(self) -> None:
        if not (self.project_root / "tortu.project").is_file():
            ...
            return
        name, ok = QInputDialog.getText(self, "New Language CSV", "File name (without .csv):")
        name = name.strip()
        if not ok or not name:
            return
        languages_dir = self.project_root / LANGUAGES_DIR
        languages_dir.mkdir(parents=True, exist_ok=True)
        dest = languages_dir / f"{name}.csv"
        if dest.exists():
